...
class XrayConfigTest {
    ...
    @Test fun tlsConfigIsPlainV2ray() {
        val n = XrayConfig.parse("$base?security=tls&sni=example.net&fp=chrome")
        val j = JSONObject(XrayConfig.build(n, 10808))
        val ob = j.getJSONArray("outbounds").getJSONObject(0)
        val stream = ob.getJSONObject("streamSettings")
        assertEquals("tls", stream.getString("security"))
        val tls = stream.getJSONObject("tlsSettings")
        assertEquals("example.net", tls.getString("serverName"))
        assertFalse(tls.has("fingerprint"))
        assertFalse(stream.has("realitySettings"))
        assertFalse(ob.getJSONObject("settings").getJSONArray("vnext").getJSONObject(0).getJSONArray("users").getJSONObject(0).has("flow"))
    }

    @Test fun directAndBlockOutboundsPresent() {
        val j = JSONObject(XrayConfig.build(XrayConfig.parse("$base?security=tls"), 10808))
        val obs = j.getJSONArray("outbounds")
        assertEquals(3, obs.length())
        assertEquals("proxy", obs.getJSONObject(0).getString("tag"))
        assertEquals("direct", obs.getJSONObject(1).getString("tag"))
        assertEquals("block", obs.getJSONObject(2).getString("tag"))
    }

    @Test fun socksInboundOnly() {
        // TUN обслуживает tun2socks в Go-обёртке, в конфиге только socks-inbound.
        val j = JSONObject(XrayConfig.build(XrayConfig.parse("$base?security=tls"), 10808)).getJSONArray("inbounds")
        assertEquals(1, j.length())
        assertEquals("socks", j.getJSONObject(0).getString("protocol"))
        assertEquals("127.0.0.1", j.getJSONObject(0).getString("listen"))
    }
    ...
    @Test fun rejectsReality() { reject("$base?security=reality&sni=example.net&pbk=test-key&sid=abcd") }
    @Test fun rejectsDuplicateParameters() { reject("$base?security=tls&security=reality") }
    @Test fun rejectsFlow() { reject("$base?security=tls&flow=xtls-rprx-vision") }
    ...
    @Test fun bothTargetsRequiredForAuto() {
        assertFalse(AutoPolicy.eligible(listOf(true, false)))
        assertTrue(AutoPolicy.eligible(listOf(true, true)))
        assertFalse(AutoPolicy.eligible(emptyList()))
    }
    @Test fun failureThresholdAndCooldown() {
        assertFalse(AutoPolicy.shouldSwitch(true, 1, 30000))
        assertFalse(AutoPolicy.shouldSwitch(true, 2, 19999))
        assertTrue(AutoPolicy.shouldSwitch(true, 2, 20000))
        assertFalse(AutoPolicy.shouldSwitch(false, 9, 999999))
    }
}
